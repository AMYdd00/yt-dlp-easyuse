...
### v1.1.0 (2025-05-01)
- ✨ **新增**：直播间卡片显示最后下载时间，直观了解后台运行状态
- 🐛 **修复**：`stop.bat` 误杀其他 Python 服务的问题，改为 PID 文件精确杀进程
- 🐛 **修复**：`run.bat` 为空文件的问题，重写为支持随时关窗口、后台继续运行
- 🐛 **修复**：`index.html` 中卡片 ID 拼接错误，导致最后下载时间不显示
- 🐛 **修复**：`config.json` 多余字段导致启动报错
- 🔧 **优化**：`get_last_download_time` 支持更多视频格式 (.mp4/.mkv/.webm/.mov/.avi/.flv)
- 🔧 **优化**：`run.bat` 和 `stop.bat` 使用 GBK 编码，中文显示正常
- 🗑️ **清理**：删除项目中无用的修复脚本和备份文件

### v1.0.0 (初始版本)
- 基于 yt-dlp 的直播回放自动监控工具
- Web 管理界面，支持频道增删、状态查看、手动下载
- Python 调度器替代传统批处理循环
